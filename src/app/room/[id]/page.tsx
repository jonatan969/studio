...
import { DRAFT_PICK_TIME, SUPER_ART_PICK_TIME, COIN_FLIP_DURATION, getPickOrder } from '@/lib/constants';
...
import { doc, updateDoc, setDoc, deleteDoc, collection, arrayUnion, writeBatch } from 'firebase/firestore';
...
export default function RoomPage() {
  ...
  const stateAdvancing = useRef(false);
  ...
  React.useEffect(() => {
    if (roomError) {
      ...
      toast({
        variant: 'destructive',
        title: 'Error al Cargar la Sala',
        description: 'No se pudo cargar la sala. Verifica los permisos de Firestore.'
      });
  ...
  const handleLeaveRoom = useCallback(async () => {
    ...
    try {
        ...
        // If there was a pick associated, delete it as well
        const pickDocRef = doc(firestore, 'rooms', roomId, 'picks', user.uid);
  ...
  React.useEffect(() => {
    ...
    if(roomData.phase === 'CANCELED' || roomData.phase === 'FINISHED') return;
    ...
    if(adminIsGone){
        shouldCancel = true;
        cancelMessage = 'El administrador ha abandonado. El draft ha sido cancelado.';
    }
    ...
    if (roomData.phase !== 'PREP') {
        ...
        if (currentNonSpectatorCount < requiredPlayerCount) {
            shouldCancel = true;
            cancelMessage = 'Un jugador abandonó. El draft ha sido cancelado.';
        }
    }
    
    if (shouldCancel && roomData.phase !== 'CANCELED') {
        updateDoc(roomRef, { phase: 'CANCELED', turnEndsAt: null, log: arrayUnion(cancelMessage) });
        return;
    }
  ...
  const handleJoin = async (team: 'team1' | 'team2' | 'spectator') => {
    ...
    try {
      ...
      await updateDoc(roomRef, { log: arrayUnion(`${user.displayName} se ha unido como ${team}.`) });
  ...
  const handleSwitchTeam = async (team: 'team1' | 'team2' | 'spectator') => {
     if (!user || !firestore || !userPlayerInfo || !roomRef) return;
    ...
    try {
        ...
        await updateDoc(roomRef, { log: arrayUnion(`${user.displayName} ha cambiado al equipo ${team}.`) });
  ...
  React.useEffect(() => {
      if (allDataLoading || !roomData || !roomRef || !user || !players || !draftPicks || !characters || stateAdvancing.current) return;
      
      const isAdmin = user.uid === roomData.adminId;
      if (!isAdmin) return;

      const advanceState = async () => {
        stateAdvancing.current = true;
        switch(roomData.phase) {
          ...
          case 'DRAFTING': {
            if (roomData.turn === undefined || !roomData.pickOrder) break;
            ...
            if (picksMadeThisTurn >= picksExpectedThisTurn) {
                ...
                if (newTurn >= roomData.pickOrder.length || totalPicksMade >= totalPlayers) {
                    await updateDoc(roomRef, { 
                        phase: 'SUPER_ART', 
                        turnEndsAt: Date.now() + SUPER_ART_PICK_TIME * 1000, 
                        currentPicker: null,
                        log: arrayUnion('¡Todos los personajes seleccionados! Hora de elegir los Super Arts.')
                     });
                } else {
                    ...
                    await updateDoc(roomRef, { 
                        ...
                        turnEndsAt: Date.now() + DRAFT_PICK_TIME * 1000,
                        log: arrayUnion(`Es el turno de ${nextTurnInfo.team === 'team1' ? roomData.team1Name : roomData.team2Name} para elegir.`)
                    });
                }
            } else if (timeLeft <= 0 && roomData.turnEndsAt && roomData.turnEndsAt !== lastProcessedTimestamp.current) {
                lastProcessedTimestamp.current = roomData.turnEndsAt;
                    ...
                    for (let i = 0; i < picksToAutoSelect; i++) {
                        ...
                        if (!randomCharacter) continue;
                         ...
                         const pickData: Omit<DraftPick, 'id'> = {
                              ...
                              team: roomData.currentPicker as TeamId,
                              pickOrder: draftPicks.length + i + 1,
                         };
                         const pickDocRef = doc(picksRef, playerToPick.uid);
                         batch.set(pickDocRef, pickData);
                         
                         await updateDoc(roomRef, { log: arrayUnion(`¡Se acabó el tiempo! ${randomCharacter.name} fue auto-seleccionado para ${playerToPick.nickname}.`) });
                    }
                    await batch.commit();
                }
            }
          ...
          case 'SUPER_ART': {
            const nonSpectatorPlayersCount = players.filter(p => p.team !== 'spectator').length;
            const picksWithSuperArtCount = draftPicks.filter(p => !!p.superArtId).length;

            if (picksWithSuperArtCount >= nonSpectatorPlayersCount) {
              await updateDoc(roomRef, { 
                  phase: 'REVEAL', 
                  turnEndsAt: null,
                  log: arrayUnion('¡Todos los Super Arts seleccionados! ¡La revelación final!')
              });
            } else if (timeLeft <= 0 && roomData.turnEndsAt && roomData.turnEndsAt !== lastProcessedTimestamp.current) {
              lastProcessedTimestamp.current = roomData.turnEndsAt;
              await updateDoc(roomRef, { 
                  phase: 'REVEAL', 
                  turnEndsAt: null,
                  log: arrayUnion('¡El tiempo de selección de Super Art ha terminado! Revelando elecciones...')
              });
            }
            break;
          }
        }
        stateAdvancing.current = false;
      };
  ...
  const handlePickCharacter = async (character: Character) => {
    if (allDataLoading || !user || !userPlayerInfo || !roomData?.pickOrder || roomData.turn === undefined || !picksRef || !draftPicks || !roomRef) return;
    ...
    try {
        ...
        await updateDoc(roomRef, { log: arrayUnion(`${userPlayerInfo.nickname} eligió a ${character.name}.`) });
  ...
  const getPhaseText = () => {
    ...
    switch (roomData.phase) {
      ...
      case 'COIN_FLIP': return `Lanzando la moneda...`;
  ...
  const myCharacterSuperArts = myPick ? superArts.filter(sa => sa.characterId === myPick.characterId) : [];
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            />
             <div className="flex items-center gap-2 self-end sm:self-center">
                ...
                {userPlayerInfo && <Button variant="destructive" size="sm" onClick={handleLeaveRoom}>
        ...
        <div className="flex-grow grid grid-cols-1 lg:grid-cols-[280px_1fr_280px] xl:grid-cols-[320px_1fr_320px] gap-4">
          ...
          <div className="flex flex-col gap-4 items-center justify-center min-h-[300px] lg:min-h-0">
            ...
            {roomData.phase === 'DRAFTING' && (
             ...
             {roomData.phase === 'SUPER_ART' && userPlayerInfo?.team !== 'spectator' && myPick && (
                <SuperArtSelector 
                  superArts={myCharacterSuperArts} 
